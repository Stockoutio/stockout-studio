(function () {
  ...
  var aim = { x: 0, y: 0 }, lastMove = -9999, lastFire = 0, lastSpawn = 0, last = 0;
  var running = false, runId = 0, started = false;
  ...
  function step(now) {
    ...
    if (now - lastMove > 1200) { var t = now / 1000; aim.x = W * 0.5 + Math.cos(t * 0.7) * W * 0.30; aim.y = H * 0.5 + Math.sin(t * 1.05) * H * 0.32; }
    ...
    for (var j = pellets.length - 1; j >= 0; j--) {
      ...
      if (hit) { var ix = enemies.indexOf(hit); if (ix >= 0) { enemies.splice(ix, 1); explode(hit.x, hit.y); } pellets.splice(j, 1); continue; }
